...
$$ q_{\bm \phi}(\bold z|\bold x^{(i)})$$는 평균과 diagonal 공분산을 가지는 정규분포이고, $$p_{\bm \theta}(\bold z)$$ 는 영벡터를 평균으로 하고 단위행렬을 공분산으로 하는 정규분포이다. 따라서 평균 벡터 j번째 성분과 공분산 j번째 대각 성분, $$\bold z$$ 의 __차원 $$J$$ 에 대해 식 \(1\)은 다음과 같이 나타낼 수 있다. 
