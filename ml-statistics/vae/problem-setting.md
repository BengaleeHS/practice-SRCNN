...
그럼 나이브 베이즈 예시에서 보았던 것 처럼 posterior를 계산할 수 있을까?  $$\bold x$$ 에 대한 $$\bold z$$ 의 조건부확률 분포\(posterior\)을 계산하고 파라미터  $$\bm \theta$$를 최대 가능도\(ML\) 방법으로 최적화하면, 주어진 $$\bold z$$ 에 대한 $$\bold x$$를 샘플링할 수 있다. 베이즈 정리에 따라
...
로 나타난다.  $$\bold z$$의 분포를 적절히 가우시안으로 가정하고 샘플링 하더라도, $$p_{\bm \theta}(\bold x|\bold z)$$를 모른다.  이미지 생성처럼 엄청나게 많은 잠재 변수를 가진 경우에 모델이 어떤 이미지를 출력할지 계산하는 것은 $$\bold z$$가 연속이고 매우 크기 때문에 알 수 없다. 무한히 적분해야하므로, 따라서 적분할 수 없다.
