...
로 나타난다.  $$\bold z$$의 분포를 적절히 가우시안으로 가정하고 샘플링 하더라도, $$p_{\bm \theta}(\bold x|\bold z)$$를 모른다.  이미지 생성처럼 엄청나게 많은 잠재 변수를 가진 경우에 모델이 어떤 이미지를 출력할지 계산하는 것은 $$\bold z$$가 연속이고 매우 크기 때문에 알 수 없다. 무한히 적분해야하므로, 따라서 적분할 수 없다. 애초에 이를 적분할 수 있으면 바로 MLE 사용하면 되지만 불가능하다.
